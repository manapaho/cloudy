...
const webpackLambdasConfig = require('../lambdas/webpack.config.babel');

/**
 * Validate the arguments.
 */
if (!yargs.argv.config) {
  console.log(`Please provide the config name to deploy: npm run deploy -- --config xxx`);
  process.exit(1);
}

if (!fs.existsSync(path.join(__dirname, `../deployments/${yargs.argv.config}.json`))) {
  console.log(`There is no ${yargs.argv.config}.json configuration file!`);
  process.exit(1);
}
 ...
 */
let compileLambdas = new Promise((resolve, reject) => {
  webpack(webpackLambdasConfig, function (err, stats) {
  ...
  console.log(`Failed to compile Lambda Functions:`, err);
  process.exit(1);
}).then(() => {
  ...
  try {
    config = JSON.parse(fs.readFileSync(path.join(__dirname, `../deployments/${yargs.argv.config}.json`), 'utf-8'));
  }
  catch (err) {
  ...
  console.log('Succeeded! TODO, now deploy/copy the websites and data and stuff');
});
